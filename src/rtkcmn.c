...
/*public function by gll*/
extern int iscontainchar(char *str,int len)
{
	int ii=0;
	for(ii=0;ii<len;ii++)
	{
		if(str[ii]==' ' || (str[ii]>='0' && str[ii]<='9'))
			continue;
		else
			return 0;
	}
...
extern int computedayweek(double year,double month,double day)
{
	int y = (int)year,m = (int)month,d = (int)day, iWeek=8;
	int a,b=0;
	...
	else
	{
		...
		else
		{
			switch(m)
			{
				case 1: if(d>32) b=1; break;
				case 2: if((a==1&&d>30)||(a==2&&d>29)) b=1; break;
				case 3: if(d>32) b=1; break;
				case 4: if(d>31) b=1; break;
				case 5: if(d>32) b=1; break;
				case 6: if(d>31) b=1; break;
				case 7: if(d>32) b=1; break;
				case 8: if(d>32) b=1; break;
				case 9: if(d>31) b=1; break;
				case 10: if(d>32) b=1; break;
				case 11: if(d>31) b=1; break;
				case 12: if(d>32) b=1; break;
            }
		}
	}
...
extern void quick_sort(double *a, int i, int j)
{
	int m, n; double k, temp;
	m = i; n = j; k = a[(i + j) / 2];
	do{
		while (a[m] < k&&m < j) m++;
		while (a[n]>k&&n > i) n--;
...
extern double stacount(double *a,int count)
{
	int mark,max;
	//int i, j, *s,k=0; double *va,reval;
	int i, j, s,k=0; double va,reval;
	//s = (int*)malloc(sizeof(int)*count); va = (double*)malloc(sizeof(double)*count);
	for (i = 0; i < count; ){
		//s[k] = 1; va[k] = a[i];
		s = 1; va = a[i];
		if (i == count - 1) break;
...
static int cmpobs1(const void *p1,const void *p2)
{
	obsd_t *q1=(obsd_t *)p1,*q2=(obsd_t *)p2;
	double tt;
...
extern void sortobs1(obs_t *obs)
{
	if(obs->n<=0)	return;
	qsort(obs->data,obs->n,sizeof(obsd_t),cmpobs1);
}
/////////////////////////////////////////////////////////////////////////////////
extern double sqrtdxy(double x,double y)
{
	double reval = (x-y)*(x-y);
	return reval;
}
