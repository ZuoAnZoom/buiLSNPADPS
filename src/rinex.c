...
extern void readrnxopt(const char *file,rnxopt_t *rnxopt)
{
	FILE *fp;
	...
	char buff[16*MAXOBSTYPE+4],*label=buff+60,str[4];
	char *type; int sys,tsys,flag=0,nsat=0,sats[MAXOBS]={0};
	//char tobs[6][MAXOBSTYPE][4];
	int in=0,ii,i,j,k,n,nt,prn,fcn; double del[3];
	const char *p;
	double ep[6];
	...
	while(fgets(buff,16*MAXOBSTYPE+4,fp)){
		if (strlen(buff)<=60) continue;
        else if (strstr(label,"RINEX VERSION / TYPE")) {
			rnxopt->rnxver =str2num(buff,0,9);
            switch (*(buff+40)) { /* satellite system */
                case ' ':
				case 'G': sys=SYS_GPS;  tsys=TSYS_GPS; break;
				case 'R': sys=SYS_GLO;  tsys=TSYS_UTC; break;
				case 'E': sys=SYS_GAL;  tsys=TSYS_GAL; break; /* v.2.12 */
			...
			setstr(rnxopt->prog,buff,20);
			setstr(rnxopt->runby,buff+20,20); continue;
        }
		else if (strstr(label,"COMMENT")) continue;
		else if(strstr(label,"MARKER NAME"         )){
			setstr(rnxopt->marker,buff,60); continue;
		}
		else if(strstr(label,"MARKER NUMBER")){
            setstr(rnxopt->markerno,buff,20); continue;
		}
		else if(strstr(label,"MARKER TYPE")){  //ver.3
        	setstr(rnxopt->markertype,buff,60); continue;
		}
		else if(strstr(label,"OBSERVER / AGENCY")){
			setstr(rnxopt->name[0],buff,20);
			setstr(rnxopt->name[1],buff+20,40);  continue;
		}
		else if(strstr(label,"REC # / TYPE / VERS")) {
			setstr(rnxopt->rec[0],buff,20);
			setstr(rnxopt->rec[1],buff+20,20);
			setstr(rnxopt->rec[2],buff+40,20); continue;
		}
		else if(strstr(label,"ANT # / TYPE")) {
			setstr(rnxopt->ant[0],buff,20);
			setstr(rnxopt->ant[1],buff+20,20); continue;
		}
		else if(strstr(label,"APPROX POSITION XYZ")) {
			for(i=0,j=0;i<3;i++,j+=14)
				rnxopt->apppos[i] = str2num(buff,j,14);
			continue;
		}
		else if(strstr(label,"ANTENNA: DELTA H/E/N")) {
			for(i=0,j=0;i<3;i++,j+=14) del[i] = str2num(buff,j,14);
			rnxopt->antdel[2] = del[0];
			rnxopt->antdel[0] = del[1];
			rnxopt->antdel[1] = del[2];  continue;
		}
		else if(strstr(label,"SYS / # / OBS TYPES")) {   //ver.3
			if(!(p=strchr(syscodes,buff[0]))){
				fclose(fp);
			...
			for(j=nt=0,k=7;j<n;j++,k+=4) {
				if(k>58) {
					if(!fgets(buff,16*MAXOBSTYPE+4,fp)) break;
					k=7;
				}
				if(nt<MAXOBSTYPE-1) setstr(rnxopt->tobs[i][nt++],buff+k,3);
			}
			*rnxopt->tobs[i][nt]='\0';
			//change beidou B1 code: 3.02 draft -> 3.02
			if(i==5) {
                for(j=0;j<nt;j++) if(rnxopt->tobs[i][j][1]=='2') rnxopt->tobs[i][j][1] = '1';
			}
			//if unknown code in ver.3, set default code
            for (j=0;j<nt;j++) {
				if (rnxopt->tobs[i][j][2]) continue;
				if (!(p=strchr(frqcodes,rnxopt->tobs[i][j][1]))) continue;
				rnxopt->tobs[i][j][2]=defcodes[i][(int)(p-frqcodes)];
				trace(2,"set default for unknown code: sys=%c code=%s\n",buff[0],
					  rnxopt->tobs[i][j]);
			}
			continue;
			...
			for (i=nt=0,j=10;i<n;i++,j+=6) {
				...
				if (nt>=MAXOBSTYPE-1) continue;
				if (rnxopt->rnxver<=2.99) {
					setstr(str,buff+j,2);
					convcode(rnxopt->rnxver,SYS_GPS,str,rnxopt->tobs[0][nt]);
					convcode(rnxopt->rnxver,SYS_GLO,str,rnxopt->tobs[1][nt]);
					convcode(rnxopt->rnxver,SYS_GAL,str,rnxopt->tobs[2][nt]);
					convcode(rnxopt->rnxver,SYS_QZS,str,rnxopt->tobs[3][nt]);
					convcode(rnxopt->rnxver,SYS_SBS,str,rnxopt->tobs[4][nt]);
					convcode(rnxopt->rnxver,SYS_CMP,str,rnxopt->tobs[5][nt]);
				}
				nt++;
			}
			*rnxopt->tobs[0][nt]='\0'; continue;
		}
		else if (strstr(label,"INTERVAL"            )) {
        	rnxopt->tint = str2num(buff,0,10); continue;
		}
		else if (strstr(label,"TIME OF FIRST OBS"   )) {
			for(i=0,j=0;i<5;i++,j+=6)
				ep[i] = str2num(buff,j,6);
			ep[5] = str2num(buff,30,13);
			rnxopt->tstart = epoch2time(ep); continue;
		}
		else if(strstr(label,"TIME OF LAST OBS")) {
        	for(i=0,j=0;i<5;i++,j+=5)
				ep[i] = str2num(buff,j,5);
			ep[5] = str2num(buff,30,13);
            rnxopt->tend = epoch2time(ep); continue;
        }
		else if(strstr(label,"END OF HEADER"))	break;
        if (++in>=1024&&type==' ') break; /* no rinex file */
	}

	i=0;
	if(((int)rnxopt->tend.time)==0){
		while(fgets(buff,MAXRNXLEN,fp)) {
			if(rnxopt->rnxver<=2.99){
				int temp = buff[32]=='G' || buff[32]=='R' || buff[32]=='E' ||
						buff[32]=='J' || buff[32]=='S' || buff[32]=='C';
				if((str2num(buff,0,3)>0) && temp)
					str2time(buff,0,26,&rnxopt->tend);
				else {
                	continue;
				}
			}
				...
				if(buff[0] == '>')
					str2time(buff,1,28,&rnxopt->tend);
				else {
                	continue;
                }
            }
