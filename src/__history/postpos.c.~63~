...
static int inputobs(obsd_t *obs, int solq, const prcopt_t *popt)
{
    ...
    if (0<=iobsu&&iobsu<obss.n) {
		...
		if (checkbrk("处理中 : %s Q=%d",time_str(time,0),solq)) {
			aborts=1;
			//showmsg_post("aborted");
			showmsg_post("中断");
			return -1;
        }
    }
                    ...
                    int mode)
{
	...
	if(popt->mode==PMODE_SINGLE) {
		rtk.rb[0]=rtk.opt.ru[0];rtk.rb[1]=rtk.opt.ru[1];rtk.rb[2]=rtk.opt.ru[2];
    }
                        ...
                        nav_t *nav, sbs_t *sbs, lex_t *lex)
{
    ...
    if (!(nav->seph=(seph_t *)malloc(sizeof(seph_t)*nav->ns))) {
		 //showmsg_post("error : sbas ephem memory allocation");
         showmsg_post("错误 ： SBAS星历内存分配失败");
                  ...
                  const sta_t *sta, const char *posfile)
{
    ...
    if (postype==1) { /* average of single position */
		if (!avepos(rr,rcvno,obs,nav,opt)) {
			//showmsg_post("error : station pos computation");
			showmsg_post("错误 : 站坐标计算");
            return 0;
        }
    }
        ...
        if (!getstapos(posfile,name,rr)) {
			//showmsg_post("error : no position of %s in %s",name,posfile);
			showmsg_post("错误 ： %s文件中没有%s的坐标位置",posfile,name);
            return 0;
        }
    }
    else if (postype==3) { /* get from rinex header */
        if (norm(stas[rcvno==1?0:1].pos,3)<=0.0) {
			//showmsg_post("error : no position in rinex header");
			showmsg_post("错误 ：在RINEX头中没有坐标位置");
                   ...
                   const filopt_t *fopt, nav_t *nav, pcvs_t *pcvs, pcvs_t *pcvr)
{
    ...
    if (*fopt->satantp&&!(readpcv(fopt->satantp,pcvs))) {
		//showmsg_post("error : no sat ant pcv in %s",fopt->satantp);
		showmsg_post("错误 ：在%s中没有卫星天线相位中心偏差",fopt->satantp);
        trace(1,"sat antenna pcv read error: %s\n",fopt->satantp);
        return 0;
    }
    /* read receiver antenna parameters */
    if (*fopt->rcvantp&&!(readpcv(fopt->rcvantp,pcvr))) {
		//showmsg_post("error : no rec ant pcv in %s",fopt->rcvantp);
		showmsg_post("错误 ：在%s中没有接收机天线相位中心偏差",fopt->rcvantp);
    ...
    if (sopt->geoid>0&&*fopt->geoid) {
        if (!opengeoid(sopt->geoid,fopt->geoid)) {
			//showmsg_post("error : no geoid data %s",fopt->geoid);
			showmsg_post("错误 ：在%s中没有geoid数据",fopt->geoid);
    ...
    if (*fopt->eop) {
        if (!readerp(fopt->eop,&nav->erp)) {
			//showmsg_post("error : no erp data %s",fopt->eop);
			showmsg_post("错误 ：在%s中没有地球自转参数值",fopt->eop);
                   ...
                   const prcopt_t *popt, const solopt_t *sopt)
{
    ...
    if (*outfile) {
        ...
        if (!(fp=fopen(outfile,"w"))) {
			//showmsg_post("error : open output file %s",outfile);
			showmsg_post("错误 ：打开输出结果文件%s",outfile);
            return 0;
        }
    }
    /* output header */
                   ...
                   char **infile, const int *index, int n, char *outfile)
{
    ...
    if (popt_.mode==PMODE_SINGLE||popt_.soltype==0) {
        ...
        if (solf&&solb) {
            ...
            showmsg_post("错误 ：内存分配失败");
			//showmsg_post("error : memory allocation");
        free(solf);
        free(solb);
        free(rbf);
        free(rbb);
                   ...
                   const char *rov, const char *base)
{
    ...
    if (ts.time!=0&&te.time!=0&&tu>=0.0) {
        if (timediff(te,ts)<0.0) {
			//showmsg_post("error : no period");
			showmsg_post("错误 ：时段不正确");
        ...
        for (i=0;;i++) { /* for each periods */
			...
			if (checkbrk("读取中    : %s",time_str(tts,0))) {
				stat=1;
                break;
            }
            for (j=k=nf=0;j<n;j++) {
